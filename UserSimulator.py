...
class UserSimulator:
    ...
    def GenerateUserGoal(self):
        self.goal = {'productname': 'any', 'city': 'any', 'numberofproducts': 'any', 'time': 'any', 'category': 'any',
                     'pricing': 'any'}
        ...
        for slot in necessarySlots:
            ...
            if slot == 'productname':
                if random.random() < 0.8:
                    ...
                    chosenValue = random.choice(slotDictionary['productname'])
                    self.goal['productname'] = chosenValue
            else:
    ...
    def SelectFirstAction(self):
        ...
        if self.goal['productname'] == 'any':
            ...
            if probability < 0.3 and self.goal['city'] != 'any' and self.goal['category'] != 'any' and self.goal[
                'pricing'] != 'any':
                ...
                elif probability > 0.4 and probability <= 0.7 and self.goal['city'] != 'any' and self.goal[
                    'pricing'] != 'any':
                    self.firstAction = {'intent': 'inform', 'informSlots': \
            ...
            if probability < 0.4:
                self.firstAction = {'intent': 'inform', 'informSlots': \
                    {'productname': self.goal['productname'], 'time': self.goal['time'],
                     'numberofproducts': self.goal['numberofproducts']}}
            else:
                # Inform restaurant and numberofproducts with 30% chance
                if random.random() < 0.5:
                    self.firstAction = {'intent': 'inform', 'informSlots': \
                        {'productname': self.goal['productname'], 'numberofproducts': self.goal['numberofproducts']}}
    ...
    def GetNextAction(self, turnCount, chosenReservation, agentAction):
        ...
        if result == SUCCESS:
            reward = round((3 * (5 * (TURN_LIMIT - turnCount+1))) / (turnCount + 1),0)
            nextAction = {'intent': 'confirm', 'informSlots': {}}
    ...
    def ChangeOptionalSlotIfNoMatches(self):
        ...
        if not self.unusedSlotValues['category'] and not self.unusedSlotValues['city'] and not self.unusedSlotValues[
            'pricing']:
            self.ResetUnusedSlotValues()
